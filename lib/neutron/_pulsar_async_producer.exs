defmodule Neutron.PulsarAsyncProducer do
  ...
  @impl true
  def handle_info(
        {:delivery_callback, result, msg_id, topic, msg, opts},
        %{config: %{callback_module: callback_module}} = state
      ) do
    try do
      callback_module.handle_delivery({result, msg_id, topic, msg, opts})
    catch
  ...
  @impl true
  def handle_call(
        {:async_produce, msg, produce_opts},
        _from,
        %{producer_ref: producer_ref} = state
      ) do
    res = PulsarNifs.async_produce(producer_ref, msg, produce_opts)
