...
static void listener_callback(pulsar_consumer_t *consumer, pulsar_message_t *message, void *ctx)
{
    ...
    enif_send(NULL, &actual_pid, env, enif_make_tuple9(env, ATOMS.atomNeutronMsg, ret_topic, p_msg_id_res, ret_part_key, ret_publish_ts, ret_event_ts, ret_redeliver_ct, ret_props, ret_payload));
...
void maybe_set_message_options(ErlNifEnv *env, pulsar_message_t *message, ERL_NIF_TERM map)
{
    ...
    if (enif_get_map_value(env, map, ATOMS.atomDeliverAfterMS, &deliver_after_ms_term) && enif_get_uint64(env, deliver_after_ms_term, &deliver_after_ms))
    {
    ...
    if (enif_get_map_value(env, map, ATOMS.atomDeliverAtMS, &deliver_at_ms_term) && enif_get_uint64(env, deliver_at_ms_term, &deliver_at_ms))
    {
    ...
    if (enif_get_map_value(env, map, ATOMS.atomEventTS, &event_ts_term) && enif_get_uint64(env, event_ts_term, &event_ts))
    {
    ...
    if (enif_get_map_value(env, map, ATOMS.atomPartitionKey, &partition_key_term) && enif_inspect_binary(env, partition_key_term, &partition_bin))
    {
        const char *partition_key = strndup((char *)partition_bin.data, partition_bin.size);
    ...
    {
        ERL_NIF_TERM cell;
        while (enif_get_list_cell(env, properties_term, &cell, &properties_term))
        {
            int arity;
            ...
            if (enif_get_tuple(env, cell, &arity, &array) && arity == 2)
            {
                ...
                ERL_NIF_TERM key_term = array[0];

                ErlNifBinary val_bin;
                ERL_NIF_TERM val_term = array[1];

                if (enif_inspect_binary(env, key_term, &key_bin) && enif_inspect_binary(env, val_term, &val_bin))
                {
                    const char *key = strndup((char *)key_bin.data, key_bin.size);
                    const char *val = strndup((char *)val_bin.data, val_bin.size);
